...
/****** Object:  StoredProcedure [custom].[DFselFacultySectionsTaught]    Script Date: 2021-07-12 12:29:44 ******/
SET ANSI_NULLS ON
GO
...
--
-- 2021-07-12 Wyatt Best:		Added Faculty Assistants via UNION.
-- =============================================
...
BEGIN
	...
	SELECT S.SectionId
		...
		,S.EVENT_LONG_NAME
		,TermId
	FROM FacultyAssistant FA
	INNER JOIN SECTIONS S
		ON S.SectionId = FA.SectionId
			AND S.EVENT_STATUS IN (
				'P'
				,'A'
				)
	INNER JOIN [custom].vwOrderedTerms OT
		ON OT.ACADEMIC_YEAR = S.ACADEMIC_YEAR
			AND OT.ACADEMIC_TERM = S.ACADEMIC_TERM
			AND OT.TermId BETWEEN (@CurTermId - @PastTerms) AND @CurTermId
	WHERE 1 = 1
		AND FA.AssistantId = dbo.fnGetPersonId(@FacultyPCID)
		AND CanAccessOverallGrades = 1
		AND CanSubmitOverallGrades = 1
